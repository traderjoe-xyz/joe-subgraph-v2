...
import { Token, LBPair, Mint, LiquidityPosition } from "../generated/schema";
import {
  getLbPair,
  getToken,
  getBundle,
...
export function handleSwap(event: Swap): void {
  ...
  // set the tokens derivedAVAX here
  // 3
...
export function handleLiquidityAdded(event: LiquidityAdded): void {
  ...
  // User
  const user = createUser(event.params.recipient);
  ...
  liquidityPosition.lbTokenBalance = liquidityPosition.lbTokenBalance.plus(
    event.params.minted.toBigDecimal()
  );
